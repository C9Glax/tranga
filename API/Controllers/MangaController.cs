...
public class MangaController(MangaContext context, ActionsContext actionsContext) : Controller
{
    ...
    /// <summary>
    /// Force re-check all failed/undownloaded chapters by deleting undownloaded records.
    /// </summary>
    ...
    [ProducesResponseType<string>(Status500InternalServerError, "text/plain")]
    public async Task<Results<Ok<string>, InternalServerError<string>>> ForceRecheckAllChapters()
    {
        try
        {
            var rowsAffected = await context.Chapters
            ...
            return TypedResults.Ok($"Deleted {rowsAffected} undownloaded chapter records. Re-check will trigger on next job run.");
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Force re-check failed/undownloaded chapters for a specific manga by deleting undownloaded records.
    /// </summary>
    /// <param name="mangaId">Manga.Key</param>
    /// <response code="200">Success message</response>
    ...
    [ProducesResponseType<string>(Status404NotFound, "text/plain")]
    [ProducesResponseType<string>(Status500InternalServerError, "text/plain")]
    public async Task<Results<Ok<string>, NotFound<string>, InternalServerError<string>>> ForceRecheckMangaChapters(string mangaId)
    {
        if (await context.Mangas.FirstOrDefaultAsync(m => m.Key == mangaId, HttpContext.RequestAborted) is not { } manga)
            return TypedResults.NotFound("Manga not found");
        ...
        try
        {
            ...
            return TypedResults.Ok($"Deleted {rowsAffected} undownloaded chapters for manga {mangaId}. Re-check will trigger on next job run.");
        }
        catch (Exception ex)
        {
            return TypedResults.InternalServerError(ex.Message);
    ...
    /// <summary>
    /// Force re-check a specific chapter by deleting its record.
    /// </summary>
    ...
    [ProducesResponseType<string>(Status404NotFound, "text/plain")]
    [ProducesResponseType<string>(Status500InternalServerError, "text/plain")]
    public async Task<Results<Ok<string>, NotFound<string>, InternalServerError<string>>> ForceRecheckChapter(string chapterId)
    {
        ...
        {
            var rowsAffected = await context.Chapters
                .Where(c => c.Key == chapterId)
                .ExecuteDeleteAsync(HttpContext.RequestAborted);

            if (await context.Sync(HttpContext.RequestAborted, GetType(), System.Reflection.MethodBase.GetCurrentMethod()?.Name) is { success: false } result)
                return TypedResults.InternalServerError(result.exceptionMessage);

            return TypedResults.Ok($"Deleted chapter record for {chapterId}. Re-check will trigger on next job run.");
        }
        catch (Exception ex)
        {
            return TypedResults.InternalServerError(ex.Message);
