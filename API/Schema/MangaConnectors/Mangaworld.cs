...
public sealed class Mangaworld : MangaConnector
{
    ...
    public override Manga[] SearchManga(string mangaSearchName)
    {
        ...
        foreach (var a in anchors)
        {
            ...
            // default status: verrà corretto se troviamo lo stato nella scheda
            var status = MangaReleaseStatus.Unreleased;
            ...
            if ((int)seriesRes.statusCode >= 200 && (int)seriesRes.statusCode < 300)
            {
                ...
                // === STATO (anche in lista, se abbiamo già aperto la scheda) ===
                var rawStatus = ExtractItalianStatus(sdoc);
                if (!string.IsNullOrWhiteSpace(rawStatus))
                    status = MapItalianStatus(rawStatus);
            }
            else
            {
            ...
            list.Add(new Manga(
                ...
                canonical,
                cover,
                status, // <-- STATO in lista se ricavato
                this,
                new List<Author>(),
                new List<MangaTag>(),
                new List<Link>(),
    ...
    public override Manga? GetMangaFromId(string mangaIdOnSite)
    {
        ...
        var detailStatus = MangaReleaseStatus.Unreleased;
        var detailRawStatus = ExtractItalianStatus(doc);
        if (!string.IsNullOrWhiteSpace(detailRawStatus))
            detailStatus = MapItalianStatus(detailRawStatus);
        ...
        return new Manga(
            ...
            url,
            cover,
            detailStatus, // <-- STATO corretto in dettaglio
            this,
            new List<Author>(),
            new List<MangaTag>(),
            new List<Link>(),
    ...
    private static string? ExtractItalianStatus(HtmlDocument doc)
    {
        ...
        var node = doc.DocumentNode.SelectSingleNode("//span[normalize-space(text())='Stato:']/following-sibling::*[1]")
        ...
        var blocks = doc.DocumentNode.SelectNodes("//*[contains(@class,'info') or contains(@class,'details') or contains(@class,'meta') or contains(@class,'attributes') or contains(@class,'list-group')]");
        if (blocks != null)
        {
            foreach (var b in blocks)
            {
                var labels = b.SelectNodes(".//dt|.//li|.//div|.//span|.//strong") ?? new HtmlNodeCollection(null);
                foreach (var lab in labels)
                {
                    var t = lab.InnerText?.Trim()?.ToLowerInvariant();
                    if (string.IsNullOrEmpty(t)) continue;
                    if (t == "stato" || t == "stato:" || t.Contains("stato"))
                    {
                        var vv = lab.SelectSingleNode("./following-sibling::*[1]")?.InnerText?.Trim()
                              ?? lab.ParentNode?.SelectSingleNode(".//a|.//span|.//strong")?.InnerText?.Trim();
                        if (!string.IsNullOrWhiteSpace(vv)) return HtmlEntity.DeEntitize(vv!);
                    }
                }
            }
        }
        ...
        var m = Regex.Match(body, @"Stato\s*:\s*([A-Za-zÀ-ÿ\s\-]+)", RegexOptions.IgnoreCase);
        return m.Success ? m.Groups[1].Value.Trim() : null;
    }

    private static MangaReleaseStatus MapItalianStatus(string s)
    {
        var k = (s ?? "").Trim().ToLowerInvariant();

        if (k.Contains("in corso") || k.Contains("ongoing") || k.Contains("attivo"))
            return MangaReleaseStatus.Continuing;

        if (k.Contains("completo") || k.Contains("concluso") || k.Contains("finito") || k.Contains("terminato") || k.Contains("completed"))
            return MangaReleaseStatus.Completed;

        if (k.Contains("in pausa") || k.Contains("pausa") || k.Contains("hiatus") || k.Contains("sospeso"))
            return MangaReleaseStatus.OnHiatus;

        if (k.Contains("droppato") || k.Contains("cancellato") || k.Contains("abbandonato") || k.Contains("cancelled") || k.Contains("interrotto"))
            return MangaReleaseStatus.Cancelled;

        // fallback neutro nel tuo enum
        return MangaReleaseStatus.Unreleased;
    }
}
