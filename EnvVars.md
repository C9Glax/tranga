...
| MAL_CLIENT_ID    | null       | Client-ID for the MyAnimeList metadata provider. Register an application at [myanimelist.net/apiconfig](https://myanimelist.net/apiconfig) (App Type "other", any redirect URL) and paste the generated Client ID here. MyAnimeList's API rejects every unauthenticated request, so while this is unset the MyAnimeList provider simply returns no search results; all other metadata providers are unaffected (docker-compose/.env key: `MALCLIENTID`) |
